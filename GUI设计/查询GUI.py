...
class app():   # 把整个GUI程序 封装在一个类里面
    ...
    def setpag(self,):    # 把界面内容放在一个一起了，便于修改
        ...
        ttk.Button(fm0, text='查询物料', command=lambda:self.find_code_GUI(self.eny_t.get())).pack(padx=20,pady=10,side='right')
        ...
        self.hbar = ttk.Scrollbar(fm2,orient='horizontal', command=self.tev.xview)
    ...
    def menu_tree(self,):    # 定义了treeview处的右键菜单内容，但菜单弹出要由post来调用
        ...
        self.menu1.add_command(label="复制编码", command=lambda:tree_copy(self.tree_code))
        self.menu1.add_separator()
        self.menu1.add_command(label="复制图号", command=lambda:tree_copy(self.tree_draw))
        self.menu1.add_separator()
        self.menu1.add_command(label="反查BOM",command=lambda : self.find_bom_GUI(self.tree_code))
        self.menu1.add_separator()
        self.menu1.add_command(label="查询子零件",command=lambda:self.find_son_GUI(self.tree_code))
    ...
    def menu_bar(self,):   # 定义菜单栏
        ...
        m_bar.add_cascade(label='导      入', menu=m_file)   #mabr上添加一个标签,链接到file_m
        ...
        for root in all_root:
            b[root[0]] = root[0]
            m_view.add_command(label=root[0] + root[1],
                               command=lambda: self.find_bom_GUI(b[root[0]]))

        m_bar.add_cascade(label=' 查看已导入的BOM ', menu=m_view)  #mabr上添加一个标签,链接到file_m
...
def find_bom(f):    # 根据编码反查使用的BOM
    ...
    rst_lv = {}
    if f in all_bom:
        rst_bom.append(all_bom[root][:])
    else:
        for root in all_bom:
            if root == 'index':
                continue             
            for num, item in enumerate(all_bom[root]):  # all_bom {root:[[lv,code,num],    ],   }
    ...
    if len(rst[0]) == 4:
        ...
        for n, key in enumerate(rst):
            for x in range(len(key[0])+1,7):
                lv_n[x] = 0
            lv_n[len(key[0])] += 1
            rst[n] = [rst[n][0]] + get_code(rst[n][1]) + [rst[n][2]] + [lv_n[len(key[0])]]
